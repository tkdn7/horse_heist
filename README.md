...
8. 탈출에 성공하면 말을 획득합니다. **내 말** 메뉴 왼쪽의 3D 미리보기로 선택한 말의 외형을 확인하고 배치·창고·보호 상태를 관리합니다.
...
높은 등급일수록 입자와 테두리 효과가 강해지고 장식이 추가됩니다. 탈취 시도는 성공·실패와 관계없이 해당 NPC 말을 소모하며, 빈 슬롯은 60초 뒤 보충됩니다.
...
PATH가 아직 반영되지 않은 PowerShell에서는 설치된 Rojo 7.7.0 실행 파일을 직접 사용합니다.

```powershell
& "$env:USERPROFILE\.rokit\tool-storage\rojo-rbx\rojo\7.7.0\rojo.exe" serve default.project.json
```

VS Code 작업도 이 프로젝트에 고정된 Rojo 7.7.0 실행 파일을 사용합니다. 버전을 올리면 작업 경로도 함께 갱신하세요.
